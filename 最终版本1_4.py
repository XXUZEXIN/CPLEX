...
class solution:
    def __init__(self,seller,buyer,part_rate):
        self.seller=seller
        self.buyer=buyer
        self.result=[]
        ...
        self.roll=0 ##历经9轮
        self.roll_num=len(part_rate)

        self.flag1=True##用来控制意向的顺序 ，True代表大的意向先行
        self.flag2=False ##用来控制从大buyer的开始还是从小的开始  True不足的时候小的开始，足的时候大的开始 这个一定要是False
        self.flag3=False#最后无意向的大的开始还是小的开始 True是小的开始
        self.flag4=False##
        self.tep=0
    ...
    def search_for_one_buyer(self,var,yixiangs,buyer_id,buyer_index):
        ...
        if max(ab)<number and sum(ab)>=number: #这代表要用2个以上仓库了 两个仓库，怎么用？先判断需要使用仓库的数量。再判断每个仓库可以的意向
            tempc=[]
            tem_number=number
            ware_num=0
            for j in ab.index:
                if tem_number>ab.loc[j]:
                    tem_number=tem_number-ab.loc[j]
                ware_num+=1
                if tem_number<=0:
                    break
            ...
            for j in ab.index:
                tem_num=ab.loc[j]
                tempd=tem_seller[tem_seller['仓库']==j]
                tempc.append([j,tem_num,sum(tempd['权重']*tempd['货物数量（张）'])/tem_num])
            tempc=sorted(tempc,key=lambda x:x[1])
            all_ware_num=[]
            all_ware_id=[]
            ind=0
            while ind<len(tempc):
                all_ware_num.append(tempc[ind][1])
                all_ware_id.append(tempc[ind][0])
                if sum(all_ware_num)>=number and len(all_ware_num)==ware_num:
                    break
                if len(all_ware_num)==ware_num and all_ware_num<number:
                    min_ind=all_ware_num.index(min(all_ware_num))
                    all_ware_num=all_ware_num[:min_ind]+all_ware_num[min_ind+1:]
                    all_ware_id=all_ware_id[:min_ind]+all_ware_id[min_ind+1:]
                ind+=1
            tem_sel=[]
            for j in all_ware_id:
                tem_sel.append(tem_seller[tem_seller['仓库']==j])
            tem_seller=pd.concat(tem_sel)
            tem_seller = tem_seller.sort_values(by=['权重1', '货物数量（张）'], ascending=self.flag4)
        else:
            tem_seller = tem_seller.sort_values(by=['仓库总数量','权重1','货物数量（张）'], ascending=self.flag4)
        # tem_seller = tem_seller.sort_values(by=['仓库总数量', '权重1', '货物数量（张）'], ascending=[False,False,True])
    ...
    def search_sameyixiang(self,emotions_and_good):
        for dis, var, sum1, rate in emotions_and_good:
            ...
            if self.tem_yixiang_index==self.tep:
                self.tem_seller=self.seller[(self.seller[thefirst]==dis) & (self.seller['品种']==var)]  ##如果能够满足第一第二意向的话，还是满足，否则直接从全量中搜索
            else:
            ...
            print("当前是第几意向：",self.tem_yixiang_index,"  第几轮：",self.roll,"  seller数量:",len(self.tem_seller),'   该意向下Buyer数量:', len(self.tem_buyer),'  买方的总数量:', sum(self.tem_buyer['购买货物数量']),'   卖方总数量:',sum(self.tem_seller['货物数量（张）']),"  应该取多少进行分配:",sum_dis)
    ...
    def sort_by_xiyang(self):
        while self.tem_yixiang_index<5:
            ...
            for i in emotions:
                ...
                emotions_and_good.append((i, "SR", tem1, round(tem1/(temp1+0.1),1)))
            ...
            if self.flag1 == True:
                ...
                emotions_and_good = sorted(emotions_and_good, key=lambda x: [x[3],x[2]])
            self.search_sameyixiang(emotions_and_good)
            self.roll+=1
            if self.roll==self.roll_num:
                self.roll=0
                self.tem_yixiang_index+=1
                print("第{}意向开始".format(self.tem_yixiang_index))
    def main(self):
        self.sort_by_xiyang()
        self.search_by_noyixiang()

# part_rate=[0.15,0.2,0.3,0.45,0.6,0.75,1]
# part_rate=[0.2,0.3,0.4,0.5,0.7,1]
# part_rate=[0.2,0.25,0.3,0.5,1]
part_rate=[0.4,0.6,1]
# part_rate=[0.4,1]
sol=solution(seller,buyer,part_rate)
sol.main()
