...
class solution:
    ...
    def search_for_one_buyer(self,var,yixiangs,buyer_id,buyer_index):
        ...
        for j, (yixiang, yixiang_value) in enumerate(yixiangs):
            ...
            if yixiang == '仓库' or yixiang == '年度':
                yixiang_value = int(yixiang_value)
            tem_seller.loc[tem_seller.index, '权重1']=tem_seller.loc[tem_seller.index, '权重1'] +(tem_seller.loc[tem_seller.index, yixiang]== yixiang_value).astype(int)*weight[self.tem_yixiang_index+1]
    ...
    def sort_by_xiyang(self):
        while self.tem_yixiang_index<5:
            ...
            for i in emotions:
                ...
                emotions_and_good.append((i, "SR", tem1, tem1/(temp1+0.1) ))
                emotions_and_good.append((i, "CF", tem2,  tem2/(temp2+0.1)))
            emotions_and_good = [i for i in emotions_and_good if i[2] > 0]
            if self.flag1 == True:
                emotions_and_good = sorted(emotions_and_good, key=lambda x: x[3])[::-1] ##越大说明越紧急
            else:
                emotions_and_good = sorted(emotions_and_good, key=lambda x: x[3])
